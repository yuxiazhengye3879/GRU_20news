...
由于实际运行时发现在线下载可能出现 `403 Forbidden`，本项目代码会优先读取仓库内本地目录：

- `20news-bydate/20news-bydate-train/`
- `20news-bydate/20news-bydate-test/`

只有在本地目录不存在时，才会回退到 `fetch_20newsgroups` 在线下载。
...
> 建议：先确认仓库中已包含 `20news-bydate` 本地数据目录。这样运行时不会走在线下载逻辑，可避免 `403 Forbidden`。
